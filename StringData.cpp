...
const char* StringData::get() const {
    return (this->_flag) ? this->_info.sStr.data : this->_info.lStr.data;
}

unsigned int StringData::size() const {
    return (this->_flag) ? this->_info.sStr.size : this->_info.lStr.size;
}
...
char& StringData::operator[](unsigned int idx) {
    ...
    return this->_flag ? this->_info.sStr.data[idx] : this->_info.lStr.data[idx];
}
